...
// We don't need ScrollTrigger for this page anymore
...
function About() {
  ...
  // Create refs for all sections we want to animate on page load
  const visionSection = useRef(null);
  const philosophySection = useRef(null);
  const chefSection = useRef(null);

  useLayoutEffect(() => {
    // This is the CORRECT animation logic that runs ON PAGE LOAD
    const ctx = gsap.context(() => {
      // A timeline to control the sequence of animations
      const tl = gsap.timeline({ delay: 0.5 }); // Start after a brief delay
      
      // Animate the vision section first
      if (visionSection.current) {
      ...
      if (philosophySection.current) {
        tl.from(philosophySection.current.children, {
          opacity: 0,
          y: 50,
          duration: 0.8,
          stagger: 0.2,
      ...
      // Finally, animate the chef section
      if (chefSection.current) {
        tl.from(chefSection.current.children, {
          opacity: 0,
          y: 50,
          duration: 0.8,
          stagger: 0.2,
  ...
  return (
    <div className="bg-black text-white">
      ...
      <div ref={philosophySection} className="bg-gray-900/50 py-24 px-10">
        <h2 className="text-5xl font-serif mb-6 text-yellow-400 text-center">Our Philosophy</h2>
